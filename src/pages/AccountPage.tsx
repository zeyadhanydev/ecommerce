  ...
  const NavItem = ({ tab, icon: Icon, label }: { tab: Tab; icon: React.ElementType; label: string }) => (
    <button
      ...
      className={`flex items-center gap-3 w-full text-left px-4 py-3 rounded-md transition-colors ${
        activeTab === tab ? 'bg-brand-gray-light text-brand-black font-semibold' : 'hover:bg-brand-gray-light text-brand-black/70'
      }`}
    >
...
const DashboardTab = ({ profile }: { profile: Profile }) => (
  <div>
    <h2 className="font-heading text-2xl mb-4">Hello, {profile.first_name}!</h2>
    ...
    return (
        <div>
            ...
            <div className="space-y-6">
                {orders.map(order => (
                    <div key={order.id} className="border border-brand-gray rounded-md p-4">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4">
                            <div>
                                <h3 className="font-semibold">Order #{order.id}</h3>
                                <p className="text-sm text-brand-black/70">Date: {new Date(order.created_at).toLocaleDateString()}</p>
                            </div>
                            <div className="text-right">
                                <p className="font-semibold">Total: €{order.total_amount.toFixed(2)}</p>
                                <p className="text-sm capitalize text-brand-black/70">Status: {order.status}</p>
                            </div>
                        </div>
                        <div className="border-t border-brand-gray pt-4">
                            {order.order_items.map((item) => (
                                <div key={item.id} className="flex items-center gap-4 py-2">
                                    <img src={item.products?.image || ''} alt={item.products?.title} className="w-16 h-16 object-cover rounded-md bg-brand-gray-light" />
                                    <div className="flex-grow">
                                        <p className="font-medium">{item.products?.title}</p>
                                        <p className="text-sm text-brand-black/70">Quantity: {item.quantity}</p>
                                    </div>
        ...
        <form className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" defaultValue={profile.first_name || ''} className="w-full px-4 py-3 border border-brand-gray rounded-md bg-brand-gray-light" readOnly />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                    <input type="text" defaultValue={profile.last_name || ''} className="w-full px-4 py-3 border border-brand-gray rounded-md bg-brand-gray-light" readOnly />
                </div>
            </div>
...
const AddressesTab = () => (
    <div>
        ...
        <p className="text-brand-black/70">Address management is not yet implemented.</p>
        <Button variant="outline" className="mt-8" disabled>Add New Address</Button>
    </div>
);
